...
class File extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @return array
     */
    public function getIntegerColumnKeys()
    {
        $integerColumnKeyList = $this->helper->getModuleConfig('general/column_to_be_integer');
        if (empty($integerColumnKeyList)) {
            return [];
        }

        return array_map('trim', explode(PHP_EOL, $integerColumnKeyList));
    }

    /**
     * @return array
     */
    public function getRoundedColumnKeys()
    {
        $roundedColumnKeyList = $this->helper->getModuleConfig('general/column_to_be_rounded');
        if (empty($roundedColumnKeyList)) {
            return [];
        }

        return array_map('trim', explode(PHP_EOL, $roundedColumnKeyList));
    }

    protected function _prepareFields(&$fields, $arrayDelim = ',')
    {
        $integerKeys = $this->getIntegerColumnKeys();
        $roundedKeys = $this->getRoundedColumnKeys();
        foreach ($fields as $key => $val) {
            if (is_array($val)) {
                $toImplode = [];
                foreach ($val as $el) {
                    if (is_numeric($el) && round($el) == $el) {
                        $el = (int)$el;
                    }
                ...
                $date = $this->_localeDate->date(new \DateTime($val));
                $fields[$key] = $date->format('Y-m-d H:i:s');
            } elseif (in_array($key, $integerKeys) && is_numeric($val) && round($val) == $val) {
                $fields[$key] = (int)$val;
            } elseif (in_array($key, $roundedKeys) && is_numeric($val)) {
                $fields[$key] = round($val, 2);
            }
        }
    }
